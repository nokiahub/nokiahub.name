...
## 한번에 여러개 commit을 revert하기
여러개 commit을 revert하고 싶을 때는 `git revert`를 여러번 사용해야 한다.
하지만 `git revert`를 여러번 사용하면 commit history가 꼬일 수 있기 때문에 `git revert`를 한번에 여러개 사용하는 방법을 알아보자.

---
title: nokia's git history
---
gitGraph
commit id: "이전 커밋"
commit id: "의존성 설치"
...
revert할 커밋이 많을 때는 커밋 이전의 hash만으로도 revert가 가능하다.
