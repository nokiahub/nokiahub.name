...
description: "Gatsby에서 Next.js로 블로그 리뉴얼했어요."
tags: ["next","next.js","gatsby to nextjs","gatsby", "rehype-pretty-code", "next-contentlayer", "mdx-code", "mdx-code-block"]
---

Gatsby로 블로그를 개발하고 운영하다보니 다음과 같은 아쉬움이 눈에 띄었습니다.

- 마크다운 안에 코드를 많이 쓰는데 코드 블럭 스타일이 못생김(테마 반영x)
- 코드블럭에 필요한 기능인 클립보드 복사, 하이라이트, 파일 이름 등을 예쁘게 표시하기가 어려움.
- 라이브러리를 설치 후 플러그인 설정을 gatsby-config 파일에 따로 설정해줘야 한다.

게다가 gatsby관련 플러그인들이 deprecated되는 경우가 많아 버전이 점점 맞지 않고, 패키지를 추가할 때 마다 플러그인이 충돌하는 경우가 많아져 커뮤니티가 방대하고 익숙한 next.js로 블로그를 개선하기로 결정했습니다.<br />
프로젝트에서 이미 사용되고 있는 A 패키지가 react 17버전에 의존성이 있는 상태로 deprecated되고, 나는 B 패키지를 새로 사용하고 싶은데 react 18버전 이상을 요구한다면, A와 B 패키지가 서로 충돌하는 경우가 많아져서 프로젝트를 진행하는데 불편함이 있었습니다.<br />

이번 포스트에서는 Next.js로 블로그를 개선하면서 가장 재미있었던 코드블럭 스타일을 개선하는 방법을 소개하겠습니다.<br />

## rehype-pretty-code로 코드블럭 스타일 개선
...
마크다운을 렌더링시켜주기 전에 tree 구조로 변경되는데, 이때 `rehype-pretty-code`를 사용하여 코드블럭을 예쁘게 만들어줄 수 있습니다.<br />
